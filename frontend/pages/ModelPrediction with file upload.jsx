...
import {
  ...
  UploadCloud, FileJson, RefreshCw,
} from "lucide-react";
 ...
 * ------------------------------------------------------------------ */
function Dashboard({ d, fileName, onReset }) {
  const b = d.best;
  const kpis = [
    ...
    <div style={{ maxWidth: 1180, margin: "0 auto", display: "grid", gap: 18 }}>
      <header className="rise" style={{ display: "flex", flexWrap: "wrap", gap: 16,
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 8, alignItems: "center" }}>
          ...
          <button onClick={onReset} title={fileName ? `Loaded: ${fileName}` : "Load a file"} style={{
            display: "inline-flex", alignItems: "center", gap: 6, cursor: "pointer",
            fontFamily: body, fontSize: 12, color: T.sub, background: T.panel,
            border: `1px solid ${T.line}`, padding: "6px 10px", borderRadius: 999 }}>
      ...
      <div className="rise" style={{ display: "grid", gap: 18,
        ...
        <Panel title="ROC curve" icon={ScatterChart}
          ...
          <RocCurve roc={d.roc} auc={b.roc_auc} />
        </Panel>
      ...
      <footer style={{ fontFamily: body, fontSize: 11.5, color: T.faint, textAlign: "center", paddingTop: 4 }}>
        Loaded from {fileName || "provided data"} · rendered at runtime — no values hardcoded.
      </footer>
    </div>
  );
}
...
/* ------------------------------------------------------------------ *
 *  Loader — drag & drop / pick a model_metrics.json
 * ------------------------------------------------------------------ */
function Loader({ onData, error }) {
  const [over, setOver] = useState(false);
  const read = useCallback((file) => {
    if (!file) return;
    const reader = new FileReader();
    reader.onload = () => onData(reader.result, file.name);
    reader.onerror = () => onData(null, file.name, "couldn't read that file");
    reader.readAsText(file);
  }, [onData]);

  return (
    <div style={{ maxWidth: 560, margin: "6vh auto 0", textAlign: "center" }}>
      <span style={{ display: "grid", placeItems: "center", width: 52, height: 52, margin: "0 auto 16px",
        borderRadius: 14, background: T.teal, color: "#fff" }}>
        <Pill size={26} strokeWidth={2.2} />
      </span>
      <h1 style={{ margin: 0, fontFamily: display, fontSize: 24, fontWeight: 700,
        letterSpacing: "-0.02em", color: T.ink }}>Medication-Risk Model — Evaluation</h1>
      <p style={{ margin: "6px 0 22px", fontFamily: body, fontSize: 13.5, color: T.sub }}>
        Drop your <code style={{ fontFamily: mono, fontSize: 12.5, color: T.tealDeep }}>model_metrics.json</code> to render the dashboard.
      </p>
      ...
      <label
        ...
        style={{
          display: "block", cursor: "pointer", padding: "40px 24px", borderRadius: 16,
          background: over ? T.tealSoft : T.panel,
          border: `1.5px dashed ${over ? T.teal : T.line}`, transition: "all .15s",
        }}>
        <input type="file" accept=".json,application/json" style={{ display: "none" }}
          onChange={(e) => read(e.target.files[0])} />
        <UploadCloud size={30} color={T.teal} style={{ marginBottom: 10 }} />
        <div style={{ fontFamily: body, fontSize: 14, fontWeight: 600, color: T.ink }}>
          Drop JSON here or click to browse
        </div>
        <div style={{ fontFamily: body, fontSize: 12, color: T.faint, marginTop: 4 }}>
      ...
      {error && (
        <div style={{ marginTop: 16, display: "inline-flex", alignItems: "center", gap: 8,
          fontFamily: body, fontSize: 12.5, color: T.coral, background: T.coralSoft,
          border: `1px solid ${T.coral}`, padding: "8px 12px", borderRadius: 10 }}>
          <AlertTriangle size={15} /> Can't read that file — {error}. Check it's the metrics JSON.
        </div>
      )}

      <p style={{ marginTop: 22, fontFamily: mono, fontSize: 11, color: T.faint }}>
        <FileJson size={12} style={{ verticalAlign: "-2px", marginRight: 4 }} />
        In your project: <span style={{ color: T.sub }}>{'import metrics from "./model_metrics.json"; <ModelDashboard data={metrics} />'}</span>
      </p>
    </div>
  );
}
...
/* ------------------------------------------------------------------ *
 *  Entry — pass `data` in your app, or use the loader standalone
 * ------------------------------------------------------------------ */
export default function ModelDashboard({ data }) {
  const [raw, setRaw] = useState(data ?? null);
  const [fileName, setFileName] = useState(data ? "provided data" : null);
  const [error, setError] = useState(null);

  const handleData = useCallback((text, name, readErr) => {
    setFileName(name || null);
    if (readErr) { setError(readErr); setRaw(null); return; }
    try {
      const parsed = typeof text === "string" ? JSON.parse(text) : text;
      derive(parsed);        // validate before committing
      setError(null);
      setRaw(parsed);
    } catch (e) {
  ...
  return (
    <div style={{ background: T.bg, minHeight: "100%", padding: "clamp(16px,3vw,32px)", fontFamily: body }}>
      <style>{`
        ...
        button:focus-visible, label:focus-within { outline: 2px solid ${T.tealDeep}; outline-offset: 2px; }
        @media (prefers-reduced-motion: reduce) { .rise { animation: none; } }
      `}</style>

      {derived
        ? <Dashboard d={derived} fileName={fileName} onReset={() => { setRaw(null); setFileName(null); setError(null); }} />
        : <Loader onData={handleData} error={error} />}
    </div>
  );
}
